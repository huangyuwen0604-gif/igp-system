...
function copyAll() {
  ...
  const txt=students.map(s=>{
    ...
    const wt=d.weeks.map(w=>`  第${w.week}週｜${w.content}｜${w.method}｜${w.result}`).join('\n');
...
function buildPrintPage(d) {
  ...
  const commentRows = `<tr><th style="width:14%;white-space:nowrap">質性評語</th><td style="white-space:pre-wrap">${commentVal || '—'}</td></tr>`;
  const overallRows = `<tr><th style="width:14%;white-space:nowrap">總評成績</th><td class="${rc(d.overallResult)}">${d.overallResult || '—'}</td></tr>`;
  ...
  <div class="print-page">
    ...
    <table class="print-week-table">
      <tr><th style="width:10%">週次</th><th style="width:48%">課程內容</th><th style="width:22%">評量方式</th><th style="width:20%">評量結果</th></tr>
      ${weekRows}
    </table>`:''}
    <div class="print-section-title">質性評語</div>
    <table class="print-table">${commentRows}</table>
    <div class="print-section-title">總評成績</div>
    <table class="print-table">${overallRows}</table>
  </div>`;
}
