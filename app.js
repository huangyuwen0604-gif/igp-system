...
const DEFAULT_CONTENT_ADJUSTMENT = '加深加廣';
...
function loadCourseFromDB() {
  ...
  el('t-year').value    = '114';
  el('t-sem').value     = c.sem || '2';
  el('t-grade').value   = c.grade;
  el('t-hours').value   = c.hours;
  el('t-teacher').value = c.teacher;

  // 儲存課程物件供 tplGo 使用（解決隱藏 textarea 設值失效）
  _pendingCourse = c;

  // 填入學習重點（若 Step 1 已可見則直接填，否則 tplGo(1) 時補填）
  if (el('t-goal')) el('t-goal').value = c.goal || '';
  ...
  if (c.weeks && c.weeks.length) {
    c.weeks.forEach(w => {
      ...
      tplWks.push({ id, wk: w.wk, co: w.co, fo: w.fo, adj: DEFAULT_CONTENT_ADJUSTMENT });
    });
  } else {
    // 沒有週次資料時給3個空白
    addTplW(); addTplW(); addTplW();
  ...
  // 自動儲存範本（確保 tpl.weeks 有資料）
  saveTpl(true);
  const weekCount = c.weeks.length;
  showMsg(`✅ 已套用「${c.label}」！共 ${weekCount} 個週次，已自動儲存。`, 'ok');
}
...
function tplGo(n) {
  ...
  // Step 1（學習重點）顯示時，確保 textarea 有值（解決 display:none 時設值失效的問題）
  if (n === 1 && _pendingCourse) {
    el('t-perf').value  = _pendingCourse.performance || '';
    el('t-lcont').value = _pendingCourse.content     || '';
    if (el('t-goal')) el('t-goal').value = _pendingCourse.goal || '';
  }
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
...
function addTplW() {
  const id = 'tw' + Date.now() + Math.random().toString(36).slice(2, 5);
  tplWks.push({ id, adj: DEFAULT_CONTENT_ADJUSTMENT });
  renderTplW();
}
...
function renderTplW() {
  ...
  tplWks.forEach(w => {
    ...
    list.appendChild(d);
    el(w.id + '-adj').value = w.adj || DEFAULT_CONTENT_ADJUSTMENT;
  });
}
...
function saveTpl(silent = false) {
  ...
  tpl = {
    ...
    weeks: tplWks.map(w => {
      // DOM에서 먼저 읽고, 없으면 tplWks 원본 값 사용
      const domWk  = el(w.id + '-wk')  ? el(w.id + '-wk').value  : null;
      const domCo  = el(w.id + '-co')  ? el(w.id + '-co').value  : null;
      const domFo  = el(w.id + '-fo')  ? el(w.id + '-fo').value  : null;
      const domAdj = el(w.id + '-adj') ? el(w.id + '-adj').value : null;
      return {
        wk:  (domWk  !== null ? domWk  : w.wk)  || '',
        co:  (domCo  !== null ? domCo  : w.co)  || '',
        fo:  (domFo  !== null ? domFo  : w.fo)  || '',
        adj: (domAdj !== null ? domAdj : w.adj) || DEFAULT_CONTENT_ADJUSTMENT,
      };
    }),
  };
...
// ===== 學生管理 =====
function initStuSelector() {
  const clsSel = el('stu-cls-sel');
  if (!clsSel || typeof STUDENT_DB === 'undefined') return;
  const classes = Object.keys(STUDENT_DB).sort();
  clsSel.innerHTML = '<option value="">-- 選擇班級 --</option>' +
    classes.map(c => {
      const grade = c[0] === '7' ? '七年級' : c[0] === '8' ? '八年級' : '九年級';
      return `<option value="${c}">${grade} ${c}班</option>`;
    }).join('');
}

function filterStuByClass() {
  const cls = v('stu-cls-sel');
  const nameSel = el('stu-name-sel');
  if (!nameSel) return;
  if (!cls || typeof STUDENT_DB === 'undefined' || !STUDENT_DB[cls]) {
    nameSel.innerHTML = '<option value="">-- 選擇學生 --</option>';
    return;
  }
  nameSel.innerHTML = '<option value="">-- 選擇學生 --</option>' +
    STUDENT_DB[cls]
      .sort((a, b) => parseInt(a.num) - parseInt(b.num))
      .map(s => `<option value="${s.name}">${s.num}號 ${s.name}</option>`)
      .join('');
}

function selectedStudentsFromDropdown() {
  const nameSel = el('stu-name-sel');
  if (!nameSel) return [];
  return [...nameSel.selectedOptions]
    .map(option => option.value)
    .filter(Boolean);
}

function addStudentsByNames(names, cls) {
  const addedIndexes = [];
  let skipped = 0;
  names.forEach(name => {
    ...
    addedIndexes.push(students.length - 1);
  });
  renderStuSelector();
  if (addedIndexes.length) selectStu(addedIndexes[0]);
  return { added: addedIndexes.length, skipped };
}

function addStuFromDB() {
  const cls  = v('stu-cls-sel');
  const names = selectedStudentsFromDropdown();
  if (!cls || !names.length) { showMsg('請先選擇班級和至少一位學生', 'err'); return; }
  const { added, skipped } = addStudentsByNames(names, cls);
  if (!added) { showMsg('選取的學生都已在清單中', 'err'); return; }
  showMsg(`✅ 已加入 ${cls}班 ${added} 位學生${skipped ? `，略過 ${skipped} 位重複` : ''}`, 'ok');
}

function addAllStuFromClass() {
  const cls = v('stu-cls-sel');
  if (!cls || typeof STUDENT_DB === 'undefined' || !STUDENT_DB[cls]) {
    showMsg('請先選擇班級', 'err');
    return;
  }
  const names = STUDENT_DB[cls].map(s => s.name);
  const { added, skipped } = addStudentsByNames(names, cls);
  if (!added) { showMsg(`${cls}班學生都已在清單中`, 'err'); return; }
  showMsg(`✅ 已加入 ${cls}班 ${added} 位學生${skipped ? `，略過 ${skipped} 位重複` : ''}`, 'ok');
}
...
function renderStuSelector() {
  ...
  let html = students.map((s, i) =>
    `<button class="stu-pill ${i === curStuIdx ? 'active' : ''}" onclick="selectStu(${i})">${s.cls ? s.cls+'班 ' : ''}${s.name}</button>`
  ).join('');
...
function selectStu(idx) {
  ...
  syncTplDisplay();
  el('comment').value = s.comment || '';
  if (el('overall-result')) el('overall-result').value = s.overallResult || '';
  if (el('affective-feedback')) el('affective-feedback').value = s.affectiveFeedback || '';
  renderStuWlist(s);
}
...
function renderStuWlist(stu) {
  ...
  tools.innerHTML = `
    ...
    <span id="assessment-tools-status" style="font-size:13px;color:#15803d;font-weight:700"></span>
  `;
...
function applyBulkAssessmentToCurrent() {
  ...
  renderStuWlist(s);
  setAssessmentToolStatus('已套用到目前學生全部週次');
...
function saveAssessmentDefaultsFromCurrent() {
  ...
  setAssessmentToolStatus('已設為此課程預設');
...
function applyDefaultsToAllStudents() {
  ...
  if (curStuIdx >= 0) renderStuWlist(students[curStuIdx]);
  setAssessmentToolStatus(`已套用到 ${students.length} 位學生`);
  showMsg(`✅ 已將課程預設套用到 ${students.length} 位學生`, 'ok');
}

function setAssessmentToolStatus(text) {
  const status = el('assessment-tools-status');
  if (!status) return;
...
function copyCurrentAssessmentToAllStudents() {
  ...
  if (students.length < 2) {
    setAssessmentToolStatus('目前只有一位學生，沒有其他學生可帶入');
    showMsg('目前只有一位學生，沒有其他學生可帶入', 'err');
    return;
  }
  saveCurStuData();
  const src = students[curStuIdx];
  const copiedMethods = (tpl.weeks || []).map((_, i) => (src.methods && src.methods[i]) || '');
  const copiedResults = (tpl.weeks || []).map((_, i) => (src.results && src.results[i]) || '');
  tpl.defaultMethods = copiedMethods.slice();
  tpl.defaultResults = copiedResults.slice();
  students.forEach(s => {
    s.methods = copiedMethods.slice();
    s.results = copiedResults.slice();
  });
  renderStuWlist(src);
  setAssessmentToolStatus(`已把 ${src.name} 的評量帶入全部學生`);
...
function saveCurStuData() {
  ...
  const s = students[curStuIdx];
  s.comment=v('comment');
  s.overallResult = v('overall-result');
  s.affectiveFeedback = v('affective-feedback');
  (tpl.weeks||[]).forEach((_,i) => {
...
function buildStuData(s) {
  return {
    ...
    stuGrade:s.cls ? s.cls+'班' : tpl.grade, teacherName:tpl.teacher,
    ...
    commentAttitude:s.comment||'', commentAcademic:'',
    commentSocial:'', commentGrowth:'',
    comment:s.comment||'',
    overallResult:s.overallResult||'',
    affectiveFeedback:s.affectiveFeedback||'',
...
function buildAllPvw() {
  saveCurStuData();
  if (!students.length) {
    el('pvw-content').innerHTML = '<div class="pvw-empty">尚未新增任何學生</div>';
    return false;
  }
  el('pvw-stu-sel').value = '';
  el('pvw-content').innerHTML = students
    .map(student => buildPrintPage(buildStuData(student)))
    .join('');
  return true;
}
...
function buildPrintPage(d) {
  ...
  const isAffectiveCourse = /情意/.test(d.courseName || '');
  ...
  const commentVal = d.comment || d.commentAttitude || '';
  const summaryRows = `
    <tr><th style="width:14%;white-space:nowrap">質性評語</th><td style="white-space:pre-wrap">${commentVal || '—'}</td></tr>
    <tr><th>總評成績</th><td class="${rc(d.overallResult)}">${d.overallResult || '—'}</td></tr>`;
  const affectiveRows = isAffectiveCourse ? `
    <div class="print-section-title">學生自評及家長回饋</div>
    <table class="print-table">
      <tr><th style="width:14%;white-space:nowrap">回饋內容</th><td style="white-space:pre-wrap">${d.affectiveFeedback || ''}</td></tr>
    </table>` : '';
  const affectiveSign = isAffectiveCourse ? `
    <div class="print-section-title">簽名欄</div>
    <div class="print-sign">
      ...
      <div class="print-sign-item"><span>資優班導師簽章</span></div>
      <div class="print-sign-item"><span>特教組長簽章</span></div>
      <div class="print-sign-item"><span>輔導主任簽章</span></div>
    </div>` : '';
  ...
  <div class="print-page">
    ...
    <div class="print-section-title">質性評語與總評</div>
...
function printAllStudents() {
  if (!buildAllPvw()) {
    showMsg('尚未新增任何學生，無法批次產生 PDF', 'err');
    return;
  }
  showMsg(`✅ 已產生 ${students.length} 位學生的列印頁，請在列印視窗選「另存為 PDF」`, 'ok');
