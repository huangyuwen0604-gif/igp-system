...
async function importExcel(input) {
  ...
  if (typeof XLSX === 'undefined') {
    showMsg('❌ SheetJS 未載入，請確認網路連線後重新整理頁面', 'err');
    input.value = '';
    return;
  }
  ...
  try {
    ...
    // 確認工作表存在
    if (!wb.Sheets['課程基本資料']) {
      showMsg('❌ 找不到「課程基本資料」工作表，請確認使用本系統提供的範本格式', 'err');
      input.value = '';
      return;
    }
    ...
    // 驗證必要欄位
    const required = ['key', 'label', 'subject', 'grade', 'hours'];
    const missing = required.filter(r => !(r in colMap));
    if (missing.length) {
      showMsg(`❌ 欄位解析失敗，缺少：${missing.join('、')}。請確認範本格式未被修改。`, 'err');
      input.value = '';
      return;
    }
    ...
    for (let r = 3; r < rows1.length; r++) {
      ...
      courseMap[key] = {
        ...
        competency:  String(row[colMap['competency']]   || '').trim()
    ...
    if (!dataRowCount) {
      showMsg('⚠️ 課程基本資料工作表沒有資料列（第4列起填入）', 'err');
      input.value = '';
      return;
    }
    ...
    filterDbCourses();
    showMsg(`✅ 匯入完成！新增 ${added} 門、更新 ${updated} 門課程，共 ${weekCount} 筆週次資料。請從下拉選單選取。`, 'ok');

  } catch (err) {
    showMsg(`❌ 解析失敗：${err.message}`, 'err');
