...
function filterDbCourses() {
  ...
  const filtered = (typeof COURSE_DB !== 'undefined' ? COURSE_DB : [])
    .filter(c => !grade || c.grade === grade || (!c.grade && !grade));
  sel.innerHTML = '<option value="">-- 選擇科目 --</option>' +
    filtered.map((c, i) => {
      const realIdx = (typeof COURSE_DB !== 'undefined' ? COURSE_DB : []).indexOf(c);
      return `<option value="${realIdx}">${c.label}</option>`;
    }).join('');
...
function previewDbCourse() {
  const idx = parseInt(v('db-course'));
  const preview = el('db-preview');
  if (isNaN(idx) || idx < 0 || !COURSE_DB[idx]) { preview.style.display = 'none'; return; }
  const c = COURSE_DB[idx];
...
function loadCourseFromDB() {
  const idx = parseInt(v('db-course'));
  if (isNaN(idx) || idx < 0 || !COURSE_DB[idx]) {
...
// ===== Excel 課程計畫匯入 =====
// 使用方式：在 HTML 加入 <input type="file" id="excel-import" accept=".xlsx" onchange="importExcel(this)">
async function importExcel(input) {
  const file = input.files[0];
  if (!file) return;
  showMsg('⏳ 正在解析 Excel...', 'ok');
  ...
  // ── 讀取「課程基本資料」工作表 ──
  const ws1 = wb.Sheets['課程基本資料'];
  if (!ws1) { showMsg('❌ 找不到「課程基本資料」工作表', 'err'); return; }
  const rows1 = XLSX.utils.sheet_to_json(ws1, { header: 1, defval: '' });

  // 找欄位標頭（第3列，index 2）
  const headerRow = rows1[2] || [];
  const colMap = {};
  headerRow.forEach((h, i) => {
    if (!h) return;
    const k = String(h).split('\n')[0].replace(' *', '').trim();
    colMap[k] = i;
  });

  const courseMap = {};
  for (let r = 3; r < rows1.length; r++) {
    const row = rows1[r];
    const key = String(row[colMap['key']] || '').trim();
    if (!key) continue;
    courseMap[key] = {
      key,
      label:      String(row[colMap['label']]      || key).trim(),
      subject:    String(row[colMap['subject']]     || '').trim(),
      grade:      String(row[colMap['grade']]       || '').trim(),
      teacher:    String(row[colMap['teacher']]     || '').trim(),
      hours:      String(row[colMap['hours']]       || '').trim(),
      competency: String(row[colMap['competency']]  || '').trim().split('、').map(s=>s.trim()).filter(Boolean),
      goal:       String(row[colMap['goal']]        || '').trim(),
      performance:String(row[colMap['performance']] || '').trim(),
      content:    String(row[colMap['content']]     || '').trim(),
      adjContent: String(row[colMap['adjContent']]  || '').trim(),
      adjProc:    String(row[colMap['adjProc']]     || '').trim(),
      ctDomain:   String(row[colMap['ctDomain']]    || '').trim(),
      ctSpecial:  String(row[colMap['ctSpecial']]   || '').trim(),
      weeks: [],
    };
  }

  // ── 讀取「週次明細」工作表 ──
  const ws2 = wb.Sheets['週次明細'];
  if (ws2) {
    const rows2 = XLSX.utils.sheet_to_json(ws2, { header: 1, defval: '' });
    const h2 = rows2[2] || [];
    const c2 = {};
    h2.forEach((h, i) => { if (h) c2[String(h).split('\n')[0].replace(' *','').trim()] = i; });
    for (let r = 3; r < rows2.length; r++) {
      const row = rows2[r];
      const key = String(row[c2['key']] || '').trim();
      if (!key || !courseMap[key]) continue;
      const wk = String(row[c2['wk']] || '').trim();
      if (!wk) continue;
      courseMap[key].weeks.push({
        wk,
        co:  String(row[c2['co']]  || '').trim(),
        fo:  String(row[c2['fo']]  || '').trim(),
        adj: String(row[c2['adj']] || '統整教學主題').trim(),
      });
    }
  }

  const imported = Object.values(courseMap);
  if (!imported.length) { showMsg('❌ 沒有解析到任何課程資料，請確認 Excel 格式', 'err'); return; }

  // 合併到現有 COURSE_DB（相同 key 則覆蓋）
  imported.forEach(c => {
    const existing = COURSE_DB.findIndex(x => x.key === c.key);
    if (existing >= 0) COURSE_DB[existing] = c;
    else COURSE_DB.push(c);
  });

  filterDbCourses();
  showMsg(`✅ 已匯入 ${imported.length} 門課程！請從下拉選單選取。`, 'ok');
  input.value = '';
}
