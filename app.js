...
function courseTopicSummary() {
  const topics = tplWks
    .map(w => {
      const co = el(w.id + '-co') ? el(w.id + '-co').value : w.co;
      return (co || '').replace(/\s+/g, ' ').trim();
    })
    .filter(Boolean)
    .slice(0, 4);
  return topics.length ? topics.join('、') : '課程核心主題';
}

function subjectGoalProfile(subject) {
  const name = subject || '本課程';
  if (/數學|理化|生物|自然|探究|研究|資訊/.test(name)) {
    return {
      concept: '建立核心概念與知識架構，能理解重要原理、符號或模型之間的關聯。',
      inquiry: '運用觀察、推理、實作或資料分析方法，解決具有挑戰性的問題。',
      expression: '能以口語、圖表、書面或作品呈現學習成果，並進行反思與修正。',
    };
  }
  if (/國文|英文|語文|閱讀|國際/.test(name)) {
    return {
      concept: '提升文本理解、語言運用與跨文本統整能力，掌握主題、觀點與表達特色。',
      inquiry: '運用閱讀策略、資料蒐集與討論探究，形成有根據的理解與個人見解。',
      expression: '能以口語、書面、簡報或創作方式清楚表達想法，並回應他人觀點。',
    };
  }
  if (/情意|生涯|獨素/.test(name)) {
    return {
      concept: '增進自我理解、情緒覺察與生涯探索，能辨識個人優勢與成長需求。',
      inquiry: '透過活動體驗、討論與任務實作，培養問題解決與規劃執行能力。',
      expression: '能分享學習歷程與反思結果，並將所學應用於生活與人際互動。',
    };
  }
  return {
    concept: '理解課程核心概念，建立有系統的知識架構與學習方法。',
    inquiry: '運用探究、討論與實作策略，解決課程相關問題並深化學習。',
    expression: '能整理學習成果、表達個人觀點，並依回饋調整後續學習。',
  };
}

function generateCourseGoal() {
  const subject = v('t-cn') || '本課程';
  const profile = subjectGoalProfile(subject);
  const topics = shortText(courseTopicSummary(), 58);
  const perfHint = shortText(v('t-perf'), 38);
  const contentHint = shortText(v('t-lcont'), 38);
  const goals = [
    `1. 透過「${topics}」等學習內容，${profile.concept}`,
    `2. 結合${perfHint || '課程學習表現'}，${profile.inquiry}`,
    `3. 依據${contentHint || '課程學習內容'}，${profile.expression}`,
  ].join('\n');
  el('t-goal').value = goals;
  showMsg('✅ 已生成三點學年目標，可再依學生需求微調', 'ok');
  syncTplDisplay();
}
